...
Shader "CustomRenderTexture/NewFullScreenOutline"
{
    Properties
    {
        ...
        // NEW: depth rejection threshold – small depth edges (grass) get ignored
        _DepthRejectThreshold("Depth Reject Threshold", Range(0, 0.05)) = 0.005
    }
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            float ComputeEyeDepthDelta(float2 uv, float2 texelSize)
            {
                ...
                for (int y = -1; y <= 1; y++)
                {
                    ...
                    for (int x = -1; x <= 1; x++)
                    {
                        ...
                        float2 offset = float2(x, y) * texelSize;
                        float d = LinearEyeDepth(SampleSceneDepth(uv + offset), _ZBufferParams);

                        maxDelta = max(maxDelta, abs(d - center));
                    }
                }
            ...
            float4 Frag(Varyings input) : SV_Target
            {
               // float mask = SAMPLE_TEXTURE2D_X(_OutlineMask, sampler_OutlineMask, input.texcoord).r;
               //  return float4(mask, mask, mask, 1);
                ...
                float centerDepthRaw = SampleSceneDepth(input.texcoord);
                if (centerDepthRaw >= 0.99999)
                    return originalColor;

                float depthDelta = ComputeEyeDepthDelta(input.texcoord, texelSize);
